...
- **ClearCast voice isolation** (`src/main/video/clearCast.ts`): FFmpeg audio chain (subsonic/high-pass → RNNoise `arnndn` with `resources/rnnoise/cb.rnnn` → gate → de-ess → speech-normalize) applied on Editor export and recording save. Testable on Linux: generate a noisy voice clip (espeak-ng + FFmpeg noise beds) into `~/Desktop/Blur Sounds Recordings`, import in the Editor, enable ClearCast, export, and compare noise floor with `ffmpeg -af volumedetect` / `showspectrumpic`. The live-mic (C#) path can't be exercised here (no mic on the VM); its preset buttons are disabled without a device.
